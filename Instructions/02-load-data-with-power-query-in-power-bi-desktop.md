...
# Power BI Desktop にデータを読み込む

**ラボの推定所要時間は 45 分です。**

このラボでは、前のラボで作成された各クエリに変換を適用します。 その後、クエリを適用し、それぞれテーブルとしてデータ モデルに読み込みます。
...
この演習では、前のラボで作成された各クエリに変換を適用します。
...
1. **SalesPersonFlag** 列ヘッダーを選択し、下矢印を選択してから **[FALSE]** を選択し、 **[OK]** をクリックします。
    
    このアクションでは、行をフィルター処理して、営業担当者である従業員のみを取得します。**
...
1. **[フィールド]** ウィンドウ (右側にあります) で、データ モデルに読み込まれた 7 つのテーブルに注目します。

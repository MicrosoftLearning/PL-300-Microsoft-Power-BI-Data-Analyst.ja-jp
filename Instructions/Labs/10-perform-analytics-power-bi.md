...
lab:
  ...
  description: このハンズオン ラボでは、Sales Exploration レポートを作成し、アニメーション化された散布図と予測機能を使って Power BI で高度な分析を実行する方法を学びます。 アニメーションによって経時的なデータ パターンを視覚化する動的な散布図の作成と、組み込みの分析機能を用いた視覚エフェクト内での直接的な将来値の予測を、実際に経験します。
  duration: 30 minutes
  level: 300
  islab: true
  primarytopics:

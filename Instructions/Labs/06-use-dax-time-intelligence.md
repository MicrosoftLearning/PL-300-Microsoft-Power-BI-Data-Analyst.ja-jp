...
lab:
  ...
  description: このハンズオン ラボでは、タイム インテリジェンス関数を使って、年初来累計 (YTD)、前年度比、その他の時間ベースの集計などの日付固有の計算を実行する DAX メジャーを作成する方法を学びます。 さまざまな期間についてデータを分析するフィルター コンテキストの操作を実際に経験し、Power BI レポートに強力なテンポラル分析を組み込めるようになります。
  duration: 15 minutes
  level: 400
  islab: true
  primarytopics:

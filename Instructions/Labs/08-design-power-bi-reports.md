...
1. キャンバスの横に **[画像の書式設定]** ペインが開きます。 このペインから、**[画像] > [参照]** を選択して、次の手順で画像ファイルを選択します。

    ![[画像の書式設定] ペイン](Linked_image_Files/08-design-power-bi-reports-format-image.png)
...
2. **[フィルター]** ペイン (**[視覚化]** ペインの左側) で、**[このページでのフィルター]** セクションに注目してください (ペインを展開して下にスクロールする必要がある場合があります)。

    ![画像 23](Linked_image_Files/08-design-power-bi-reports_image44.png)

3. **[データ]** ペインから、`Product | Category` フィールドを **[このページでのフィルター]** ウェルにドラッグします。
...
4. フィルター カード内の右上にある矢印を選択して、カードを折りたたみます。

5. `Product` テーブルの次の各フィールドを **[このページでのフィルター]** ウェルの `Category` カードのすぐ下に追加し、それぞれを折りたたみます。
...
1. **[複数の行カード]** 視覚エフェクトをページに追加し、スライサーの右側でページの残りの幅を埋めるようにサイズと位置を変更します。
    ...
    > _**[ビジュアル化]** ウィンドウに **[複数行カード]** ビジュアルが表示されない場合は、ビジュアルの下にある省略記号 (**...**) を選択し、**既定のビジュアルを復元**します。**[複数行カード]** が選択できるようになります。_

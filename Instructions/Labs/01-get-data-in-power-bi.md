...
lab:
  ...
  description: このハンズオン ラボでは、さまざまなデータ ソース (SQL Server など) への Power BI Desktop の接続とデータのインポートを実際に経験します。 Power Query を使ってソース データをプレビューし、データ プロファイル手法を適用してデータの特性の理解とデータ品質の評価を行う方法を学びます。 この基本的なラボでは、Power BI でのデータ取得と初期データ評価に不可欠なスキルを身に付けます。
  duration: 30 minutes
  level: 300
  islab: true
  primarytopics:

...
1. リボンの下の警告メッセージに注目してください。 "このメッセージは、クエリが、モデル テーブルとしての読み込みに適用されていないことを警告しています。クエリは、このラボの後半で適用します。"**
    
    警告メッセージを無視するため、警告メッセージの右側にある **[X]** を選びます。**

1. ファイルのコピーを作成するため、**[ファイル] > [名前を付けて保存]** の順に移動し、**D:\Allfiles\MySolution** フォルダーに保存します。

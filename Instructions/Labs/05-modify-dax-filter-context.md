...
lab:
  ...
  description: このハンズオン ラボでは、CALCULATE 関数を使ってフィルター コンテキストを操作する高度な DAX メジャーを作成する方法を学びます。 計算にフィルターを適用する方法の制御を実際に経験し、レポートで既定のフィルターの動作をオーバーライドまたは変更できる、より高度で柔軟なメジャーを作成できるようになります。
  duration: 30 minutes
  level: 400
  islab: true
  primarytopics:
...
1. **モデル ビュー**で、3 つの新しいメジャーを _Ratios_ という名前の表示フォルダーに配置します。

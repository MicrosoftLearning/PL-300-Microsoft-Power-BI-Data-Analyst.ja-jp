...
1. 行列の視覚エフェクトのフィールドを構成するには、 **[フィールド]** ペインから **Region \| Regions** 階層をドラッグして、視覚エフェクト内にドロップします。
...
1. **[フィールド]** ウィンドウで、**Sales All Region** メジャーが選択されていることを確実にし (選択すると、背景は濃い灰色になります)、数式バーでメジャー名と数式を次の数式に置き換えます。
